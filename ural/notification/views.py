...
def arrayCars(cars):
    array=[]
    for i in cars:
        car = VCar(_id=i.car.id, _name=i.car.car, _capacity=i.car.capacity, _volume=i.car.volume, _length=i.car.length, _width=i.car.width, 
                   _height=i.car.height, _where_from=i.car.where_from, _where=i.car.where, _ready_from=i.car.ready_from, _ready_to=i.car.ready_to, 
...
def my_responses(request):
    if request.POST: #удаление запроса
        if request.POST.get('obj_cargo_id'):
            ...
            if notifyCar.objects.get(id=obj_id) != None:
                obj = notifyCar.objects.get(id=obj_id)
                obj.delete()
